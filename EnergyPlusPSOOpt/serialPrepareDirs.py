...
def parseInputSpecificationFile( filename ):
  ...
  for line in spec_file.xreadlines():
    ...
    if param_parsing_mode:
      ...
      elif name == "MaxConstantBestParticleIterations":
        run_parameters["MaxConstantBestParticleIterations"] = int(value)
      elif name == "MaxConstantBestSolutionIterations":
        run_parameters["MaxConstantBestSolutionIterations"] = int(value)
      else:
    ...
    if "MaxConstantBestParticleIterations" not in run_parameters.keys():
      run_parameters["MaxConstantBestParticleIterations"] = -1

    if "MaxConstantBestSolutionIterations" not in run_parameters.keys():
      run_parameters["MaxConstantBestSolutionIterations"] = -1
...
NConstantSolutionIterations = run_params["MaxConstantBestSolutionIterations"]
...
pso_data.OutputFile = "ParticlePositions.csv"
...
pso_data.max_constant_best_particle_iterations = NConstantParticleIterations
pso_data.max_constant_best_solution_iterations = NConstantSolutionIterations

pso_data.num_iters_particle_is_best = 0
pso_data.num_iters_solution_is_best = 0
...
# Write all the initial data 
epopt.appendColumnDataToDisk( pso_data.OutputFile, pso_data.canonical_parameters)
epopt.appendParticlePositionsToDisk( pso_data.OutputFile, pso_data.current_iteration, \
                                       pso_data.particle_list)
